import { Undefined } from "@core/shared/domain/nullable";
...
export class GenreFakeBuilder<TBuild = any> {
  ...
  private _genre_id: Undefined<PropOrFactory<GenreId>> = undefined;
  ...
  withGenreId(valueOrFactory: Undefined<PropOrFactory<GenreId>>) {
  ...
  build(): TBuild {
    const Genres = new Array(this.countObjs).fill(undefined).map((_, index) => {
      ...
      const genre = new Genre({
        genre_id: !this._genre_id
          ? undefined : this.callFactory(this._genre_id, index),
        name: this.callFactory(this._name, index),
        categories_id: new Map(categoriesId.map(category_id => [category_id.id, category_id])),
    ...
    return this.countObjs === 1 ? (Genres[0]) : Genres;
  }
