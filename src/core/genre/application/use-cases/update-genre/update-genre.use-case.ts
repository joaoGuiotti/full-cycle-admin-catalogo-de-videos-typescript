...
export class UpdateGenreUseCase
  ...
  constructor(
    ...
    private categoriesIdExistsInStorageValidator: CategoriesIdStorageValidator,
  ) { }
  ...
  async execute(input: UpdateGenreInput): Promise<GenreOutput> {
    ...
    input.is_active === true && (genre.activate());
    input.is_active === false && (genre.deactivate());

    const notification = genre.notification;

    if (input.categories_id) {
      const [categoriesId, errorsCategoiesId] = (
        await this.categoriesIdExistsInStorageValidator.validate(input.categories_id)
      ).asArray();
      categoriesId && genre.syncCategoriesId(categoriesId);
      errorsCategoiesId && notification.setError(
        errorsCategoiesId.map((e) => e.message),
        'categories_id',
      );
    }
    ...
    const categoires = await this.categoryRepo.findByIds(
      Array.from(genre.categories_id.values())
    );

    return GenreOutputMapper.toOutput(genre, categoires);
  }
